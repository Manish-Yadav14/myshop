...
import { clearCart,addItemToCart } from "../slices/cartSlice";
...
function MyCart() {
  ...
  const getCart = async () => {
    try {
      const result = await axios.get('http://localhost:3000/cart/get-cart');
      setCart(result.data[0].items);
      // dispatch(addItemToCart(result.data[0].items))
    } catch (error) {
